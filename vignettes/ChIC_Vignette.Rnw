...
The package comes with a large reference 
...
\newpage
To run ChIC the user has to provide two bam files: one for ChIP and 
...
ChIC provides three wrapper functions that are used to calculate 
a comprehensive set of metrics: \textit{qualityScores\_EM()}, 
\textit{qualityScores\_GM()} and \textit{qualityScores\_LM()}.


\section{ENCODE Metrics (EM) based on sharp-peak profiles and 
cross-correlation analysis}

\textit{qualityScores\_EM()} is a wrapper function that reads the 
...
genome annotation 
used. ChIC currently supports hg19 and mm9. \textit{qualityScores\_EM()} 
returns a number of QC-metrics, 
...
\textit{qualityScores\_EM()} executes the following steps:
\begin{enumerate}
...
For more details about the individual steps see the "Useful functions" 
section.

\newpage
\section{Useful functions}
The following code chunks are automatically 
executed by the \textit{qualityScores\_EM()} wrapper function. 
...
\textit{getCrossCorrelationScores} takes QC-metrics from 
...
required by \textit{getCrossCorrelationScores} 
is the binding-characteristics, calculated using 
\textit{spp::get.binding.characteristics} function. The 
...
The output of the \textit{getCrossCorrelationScores} function is 
...
The data is processed further by using \textit{removeLocalTagAnomalies} 
...
The last set of QC-metrics calculated by \textit{qualityScores\_EM()} 
...
The last step executed in \textit{qualityScores\_EM()} 
...
in the fuctions 
\textit{metagenePlotsForComparison}, \textit{plotReferenceDistribution} 
and \textit{predictionScore}.
...
and to get the respective LM QC-values for the non-scaled profiles 
around the TSS and TES.
...
and to get its respective LM QC-values:
