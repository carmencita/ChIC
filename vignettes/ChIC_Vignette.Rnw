...
bam file reads for chromosome 2 only. Therefore we are loading input and chip data 
...
data("chr2Bam", package = "ChIC.data", envir = environment())
chipBam=chr2Bam
data("chr2BamInput", package = "ChIC.data", envir = environment())
inputBam=chr2BamInput
@
...
<<geneBody>>
@
 \caption{Metagene profiles: Signal enrichment for ChIP and Input along the gene body.}
 \label{fig:geneBody}
\end{figure}
...
\caption{Normalized metagene profile: signal enrichment for ChIP over Input along the gene body.}
\label{fig:norm}
\end{figure}
...
@
 ...
 \caption{Enrichment profile and QC-metrics can be plotted against the pre-computed 
profiles of the compendium. The metagene profile shows the sample signal (red line) 
for the input when compared to the compendium mean signal (black line) 
and its 2x standard error (blue shadow). }
...
for the ChIP (A) and the enrichment (chip over input) (B) when compared 
