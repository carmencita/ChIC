...
distribution. It introduces local enrichment profile metrics (LM), 
specifically designed to assess the quality for broad binding profiles. 
LM are derived from meta-gene profiles and take into account the 
local ChIP-seq enrichment profile. Additionally ChIC also computes 
previously published ENCODE metrics (EM) and global enrichment 
profile metrics (GM).

The package allows to calculate QC-metrics and to perform quality 
control analysis with a single command, whereas step by step 
functions are also available for more experienced users. 
The package comes with a large reference compendium of 
precomputed QC-metrics from public ChIP-seq samples. 
...
For timing reasons in this tutorial we will start from a smaller 
example dataset available in the datapackage "ChIC.data". 
The example dataset are reads for a subset of chromosomes that can 
