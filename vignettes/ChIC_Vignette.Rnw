...
%%[link](https://www.encodeproject.org/search/?type=File&file_format=bam "Title")
%%\href{https://www.encodeproject.org/search/?type=File&file_format=bam}{https://www.encodeproject.org/search/?type=File&file_format=bam}
...
## dataDirectory: path to the directory where the bam files are 
...
input.data=readBamFile(inputName,dataPath)
\end{verbatim}
