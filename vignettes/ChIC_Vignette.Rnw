...
## get bam file
setwd(tempdir())
chipName="/lustre/data/FF/Carmen/BitBucket/ENCFF000BLL"
inputName="/lustre/data/FF/Carmen/BitBucket/ENCFF000BKA"
@
...
filepath=tempdir()
setwd(filepath)
path="https://www.encodeproject.org/files/ENCFF000BLL/@@download/"

chipName="ENCFF000BLL"
command=paste("wget",path, chipName,sep=" ")
system(command)

inputName="ENCFF000BKA"
command=paste("wget",path, inputName,sep=" ")
system(command)
@
...
chip_binding.characteristics<-spp::get.binding.characteristics(
    chip.data, srange=c(0,500), bin=5,accept.all.tags=TRUE)

input_binding.characteristics<-spp::get.binding.characteristics(
    input.data, srange=c(0,500), bin=5,accept.all.tags=TRUE)

## calculate cross correlation QC-metrics
crossvalues_Chip<-getCrossCorrelationScores(chip.data, 
chip_binding.characteristics, read_length=36,savePlotPath=NULL)

crossvalues_Input<-getCrossCorrelationScores(input.data, 
input_binding.characteristics, read_length=36,savePlotPath=NULL)
@
...
\caption{(A) Cross-correlation plot for sample "ENCFF000BLL". (B) Fingerprint 
plot of sample ENCFF000BLL and its input ENCFF000BKA. }
\label{fig:1}
\end{figure}
...
chrl_final=intersect(names(chip.data$tags),
    names(input.data$tags))
...
\textit{getPeakCallingScores} \cite{Chicpaper}.
...
Fingerprint plot (Figure \ref{fig:1}) and returns a list 
...
densityInput=smoothedInput,savePlotPath=getwd())
@
...
\begin{figure}
\centering
.\includegraphics{MetaGenePlots.pdf}
\caption{
(A) Scaled metagene plots for the ChIP and input and (B) the normalized profile.}
...
Meta_Result=createMetageneProfile(
    ...
    smoothedInput,tag.shift=finalTagShift)
@
...
\begin{itemize}
    ...
    compendium values
    \textit{predictionScore} to obtain a single quality score from the previously 
    computed QC-metrics 
\end{itemize}
...
metagenePlotsForComparison(data=Meta_Result$geneBody,
    chrommark="H3K36me3", tag="geneBody", savePlotPath=getwd())
metagenePlotsForComparison(data=Meta_Result$TSS,
    chrommark="H3K36me3", tag="TSS", savePlotPath=getwd())
@
...
\begin{figure}
\centering
.\includegraphics{MetaProfileComparison.pdf}
\caption{
Enrichment profiles and QC-metrics can be plotted against the pre-computed 
