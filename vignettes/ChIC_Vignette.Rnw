...
(ID: ENCFF000BKA) (https://www.encodeproject.org/). 
...
previously calculated using \textit{spp::get.binding.characteristics} function. 
binding.characteristics provides information about the 
...
should be saved. If not given the plot will be forwarded to stdout. 
