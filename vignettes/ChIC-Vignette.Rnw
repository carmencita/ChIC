...
\textbf{PLEASE NOTE:} In the function 
\textcolor{airforceblue}{metagenePlotsForComparison()}
as well as in \textcolor{airforceblue}{plotReferenceDistribution} and
\textcolor{airforceblue}{predictionScore}, it is necessary to specify
the expected chromatin mark or transcription factor. For more details 
see subsection "Available chromatin marks and factors".
To use the "predictionScore" function on transcription factors the 
keyword "TF" in 
the "target" argument can be used to invoce the generic transcription 
factor model.
