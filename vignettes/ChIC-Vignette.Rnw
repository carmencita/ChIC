...
\addtocounter{enumi}{8}
   \item The metagene profile around the gene body for ChIP reads density compared to mean profile (+/-2stdErr) for the same chromatin mark samples in the reference compendium
   ...
   \item The TSS metagene profile for ChIP reads density compared to mean profile (+/-2stdErr) of the reference compendium
...
These \texttt{"chipSubset"} and \texttt{"inputSubset"} objects are also preloaded and available in the companion \texttt{"ChIC.data"} package.
...
\subsubsection{Calculate QC-metrics from CrossCorrelation analysis}
...
\subsubsection{Remove anomalies in the read distrbution}
...
\subsubsection{Calculate QC-metrics from peak calls}
...
\subsubsection{Profile smoothing}
