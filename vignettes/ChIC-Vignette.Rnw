...
\noindent \textcolor{airforceblue}{qualityScores\_EM()} is a wrapper function that reads the 
...
the output measures as ENCODE Metrics EM.
...
\textcolor{airforceblue}{qualityScores\_EM()} produces the Cross-correlation plot 
(see Figure \ref{fig:1}) and returns a number of 
...
\textcolor{airforceblue}{qualityScores\_EM()} executes the following single steps:
\begin{enumerate}
...
executed within the \textcolor{airforceblue}{qualityScores\_EM()} wrapper function. 
...
\textcolor{airforceblue}{getCrossCorrelationScores()} calculates QC-metrics from 
...
The wrapper \textcolor{airforceblue}{qualityScores\_GM()} reproduces the so-called 
...
\textcolor{airforceblue}{createMetageneProfile()} creates the unscaled single-point and 
...
for the TSS and transcription end site, and a scaled whole gene metagene profile 
...
are used to plot the scaled profile and returns the respective LM values:
...
The \textcolor{airforceblue}{metagenePlotsForComparison()} 
