...
\usepackage{sectsty}
\sectionfont{\color{RoyalBlue}}  % sets colour of subsection
\subsectionfont{\color{BlueGreen}}  % sets colour of sections
\usepackage{PTSerif} % Use the Paratype Serif font

\usepackage{Sweave}
\DefineVerbatimEnvironment{Sinput}{Verbatim} {xleftmargin=1.3em, 
frame=single}%, label=Input, labelposition=topline}
\DefineVerbatimEnvironment{Soutput}{Verbatim}{xleftmargin=1.3em,
...
% CC BY-NC-SA 3.0 (http://creativecommons.org/licenses/by-nc-sa/3.0/)
    ...
    \textbf{\LARGE ChIP-seq quality Control}\\[\baselineskip] % First title line
    %\textbf{\LARGE }\\[\baselineskip] % First title line
    {\textcolor{RoyalBlue}{\Huge \textbf{ChIC} }}\\[\baselineskip] % Main title line which draws the focus of the reader
    ...
    \includegraphics[width = 40mm]{ChIC_Logo}
...
functions allow to perform the analysis with a single command, 
...
Key features are the calculation, visualization and creation of summary 
...
finally a random forest model to compute a single score for quality control.
...
a comprehensive set of metrics: qualityScores\_EM(), 
qualityScores\_GM() and qualityScores\_LM().


\section{ENCODE Metrics (EM) based on sharp-peak profiles and 
cross-correlation analysis}

\noindent qualityScores\_EM() is a wrapper function that reads the 
...
the output measures as ENCODE Metrics.
...
(ChIP) and one for the control (Input). The read length (read\_length parameter) 
can be taken from the bam file itself. The 'mc' parameter is set to 1 
per default, when changed it triggers the parallelization of a few processes 
and speeds up the calculations. 'annotationID' refers to the 
genome annotation used. ChIC currently supports hg19 and mm9. 
qualityScores\_EM() returns a number of QC-metrics, 
...
qualityScores\_EM() executes the following steps:
\begin{enumerate}
...
\noindent For more details about the individual steps see the 
"Useful functions" section.

\newpage
\section{Useful functions}
The following code chunks are automatically 
executed by the qualityScores\_EM() wrapper function. 
...
The first step in the qualityScores\_EM() function 
...
getCrossCorrelationScores() takes QC-metrics from 
the cross-correlation analysis and other general metrics, e.g. the 
non-redundant fractions of mapped reads. An important parameter 
required by getCrossCorrelationScores() 
is the binding-characteristics, calculated using 
spp::get.binding.characteristics() function. The 
...
plot (as pdf) should be saved. If not provided the plot will be 
forwarded to default DISPLAY. An example of a cross-correlation profile 
is shown in Figure \ref{fig:1}.

\noindent The output of getCrossCorrelationScores() is 
a list with calculated QC-metrics, from which we have to save 
tag.shift value for further steps:
...
The data is processed further by using removeLocalTagAnomalies() 
...
The last set of QC-metrics calculated by qualityScores\_EM() 
...
The last step executed in qualityScores\_EM() 
...
\subsection{Available chromatin marks and factors}
This usefull function lists all chromatin marks and 
transcription factors that are available for the comparison analysis 
in the fuctions metagenePlotsForComparison(), plotReferenceDistribution() 
and predictionScore().
...
The  function qualityScores\_GM() reproduces the so-called 
...
Metagene profiles show the signal enrichment around a region of interest like 
...
The normalized profile (Figure \ref{fig:MetaGenePlotNorm}) 
...
\noindent createMetageneProfile() creates the metagene profiles for scaled 
...
\noindent The objects in "Meta\_Result" can be used to create the final 
metagene plots and to get the respective LM QC-values for the 
non-scaled profiles around the TSS and TES.
...
\noindent The "geneBody" object can be used to plot the scaled metagene 
profile and to get its respective LM QC-values:
...
\begin{itemize}
    ...
    with the compendium
    \item \textit{plotReferenceDistribution} to compare a singl QC-metric 
...
The metagenePlotsForComparison() function is used to compare the 
...
(blue shadow). Examples are shown in Figures \ref{fig:MetaProfileComparison} 
and \ref{fig:MetaProfileComparisonInput}.
