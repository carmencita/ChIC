...
\paragraph{ChIP target categories - model selection in \texttt{chicWrapper()}}
...
\paragraph{ChIP target  - reference compendium subset selection for summary plots in \texttt{chicWrapper()}}
...
Then the input BAM files can be read into R objects storing the aligned reads positions and their quality scores into a \texttt{"taglist"} object as defined by the spp package \cite{Kharchenko2008}.
...
%%%%%%   THIS IT NO MORE IN LINE WITH THE CURRENT CODE, WE NEED TO PRE-SAVE A SMALLER BAM FILE CONTAINING ONLY THE READS THAT WE NEED FOR THE TEST CASE TO ACTUALLY RUN THE FULL REPORT  %%%%%
\textbf{PLEASE NOTE:} In order to comply with time limits for example code chunks to be executed when compiling Vignettes for Bioconductor and R packages, from now on we will actually use a smaller subset of these data input. Namely, for the subsequent analysis steps we will use only chromosomes 17, 18 and 19 from the samples described above.
...
These \texttt{"chipSubset"} and \texttt{"inputSubset"} objects are also available for preloading in the companion \texttt{"ChIC.data"} package.
...
<<eval=FALSE, label=Fingerprint, fig=TRUE, echo=TRUE>>=
<<Fingerprint>>
@
