...
are used to plot the scaled profile and return the respective LM values:
...
\begin{figure}[htbp]
    \centering
<<eval=TRUE, label=aba, fig=TRUE, echo=TRUE>>=
...
\caption{The metagene profile shows the enrichment (chip over input) 
