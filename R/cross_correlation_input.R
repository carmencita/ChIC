...
#inputIndex=as.integer(arg[7])
#inputname=sampleinfo$Filename[inputIndex]
path=arg[6]
inputname=arg[7]
print(inputname)
...
binding.characteristics<-get.binding.characteristics(input.data, srange=estimating_fragment_length_range, bin=estimating_fragment_length_bin, cluster=cluster)
#save(binding.characteristics,file=paste("/data/FF/Carmen/Pipeline/binding_ch",inputname,".RData",sep=""))
#load(file.path(datadir, paste("binding_ch",inputname,".RData",sep="")))

print("Cross correlation plot")
f_plot(binding.characteristics$cross.correlation,inputname,plotname="CrossCorrelation",xlab="strand shift",ylab="cross-correlation",line=binding.characteristics$peak$x)

print(binding.characteristics$peak$x)


###step 1.2: Phantom peak and cross-correlation
#read_length<-sampleinfo[inputIndex,"ReadLength"]
print("Estimating fragment lengths")
phantom.characteristics<-get.binding.characteristics(input.data, srange=PhantomPeak_range, bin=PhantomPeak_bin, cluster=cluster)
print(inputname)
print(phantom.characteristics$peak$x)
#save(phantom.characteristics,file=paste("/data/FF/Carmen/Pipeline/phantom_ch",inputname,".RData",sep=""))
#load(file.path(datadir, paste("binding_ch",inputname,".RData",sep="")))
...
binding.characteristics_cross.correlation_y_smoothed<-caTools::runmean(binding.characteristics$cross.correlation$y, k=as.integer(cross_correlation_smoothing_k))
...
###4 NRF calculation
ALL_TAGS<-sum(sapply(input.data$tags, length))
UNIQUE_TAGS<-sum(sapply(lapply(input.data$tags, unique), length))
UNIQUE_TAGS_nostrand<-sum(sapply(lapply(input.data$tags, FUN=function(x) {unique(abs(x))}), length))
...
names(input.dataNRF)<-NULL
input.dataNRF<-paste(nomi, input.dataNRF, sep="")

if (ALL_TAGS > 10e6) {
   UNIQUE_TAGS_LibSizeadjusted<-round(mean(sapply(1:100, FUN=function(x) {
     return(length(unique(sample(input.dataNRF, size=10e6))))
   })))
} else {
   UNIQUE_TAGS_LibSizeadjusted<-round(mean(sapply(1:100, FUN=function(x) {
     return(length(unique(sample(input.dataNRF, size=10e6, replace=TRUE))))
   })))
}

NRF_LibSizeadjusted<-UNIQUE_TAGS_LibSizeadjusted/10e6

STATS_NRF=c(Sample=inputname, ALL_TAGS=ALL_TAGS, UNIQUE_TAGS=UNIQUE_TAGS, UNIQUE_TAGS_nostrand=UNIQUE_TAGS_nostrand, NRF=NRF, NRF_nostrand=NRF_nostrand, NRF_LibSizeadjusted=NRF_LibSizeadjusted)
...
N1<-sum(sapply(input.data$tags, FUN=function(x) {
...
Nd<-sum(sapply(lapply(input.data$tags, unique), length))
PBC = N1/Nd
...
################RESULTS
outname=paste(outputdir,"/input_",inputname,".results",sep="")
file.remove(outname)
#sampleIndex inputIndex
