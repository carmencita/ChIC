...
{	
	if (go=="TSS")
	{
		file.metaprofili<-paste(storagedir,"/one.point.scalingTSS_",datafilename, ".RData", sep="")
		load(file.metaprofili)
		gd.gen=gd.gen_TSS
	}else{
		
		file.metaprofili<-paste(storagedir,"/one.point.scalingTES_",datafilename, ".RData", sep="")
		load(file.metaprofili)
		gd.gen=gd.gen_TES
	}
	...
	#chip_genelist= chip_genelist_pre[which((rownames(chip_genelist_pre) %in% filterGenes)),]
	chip_genelist= chip_genelist_pre

	input_preMeans_pre<-log2(do.call(rbind,gd.gfp)+psc)
	#input_preMeans= input_preMeans_pre[which((rownames(input_preMeans_pre) %in% filterGenes)),]
	...
	pdf(file=paste(plotsdir,"/NOnormalizaztion_",go,"_",TFname,"_",datafilename , ".pdf", sep=""))
	...
	pdf(file=paste(plotsdir,"/Normalized_",go,"_",TFname,"_", datafilename,  ".pdf", sep=""))
	    par(mar = c(3.5,3.5,2.0,0.5), mgp = c(2,0.65,0), cex = 1);
