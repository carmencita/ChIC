...
chance <- function() {
  ...
  # Need to define the parameteres for the function. As of now, they are defined
  # in a separate script (GlobalParameters.R), that is not really a function. We
  # should never run code at the top-level of a package. The code should only
  # contain functions. Default parameters should be defined for each function
  # separately.

  # It would be good to explicitely refer to the function names, like
  # girafe::some_function() everywhere in the code and list dependencies in
  ...
  # In general, we should never use library(), require() or source() inside the
  # functions of the package.
  # See http://r-pkgs.had.co.nz/r.html#r-differences
